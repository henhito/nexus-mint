    ...
    <div className="glass rounded-2xl p-6">
      ...
      <div className="space-y-2">
        {checks.map((item) => (
          <div
            key={item.label}
            className="flex items-start gap-3 p-3 rounded-lg bg-foreground/[0.02] hover:bg-foreground/[0.04] transition-colors"
          >
            {item.status === "active" ? (
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-foreground/40 mt-0.5">{item.detail}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 p-4 rounded-lg bg-purple-500/5 border border-purple-500/10">
        ...
        <ol className="text-xs text-foreground/60 space-y-1.5 list-decimal list-inside">
          ...
          <li>Deploy backend function: <code className="bg-foreground/5 px-1 rounded">base44 functions deploy</code></li>
